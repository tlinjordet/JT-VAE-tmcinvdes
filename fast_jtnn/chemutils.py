...
def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
    ...
    for amap in all_attach_confs:
        ...
        if smiles in cand_smiles or check_singleton(cand_mol, node, neighbors) is False:
            continue
        cand_smiles.add(smiles)
...
def check_aroma(cand_mol, ctr_node, nei_nodes):
    ...
    def get_nid(x):
        return 0 if x.is_leaf else x.nid
...
def is_valid_smiles(smiles):
    if smiles.count("Li") > 1:
        return False

    if smiles.count("Ir") > 1:
        return False

    if smiles.count("Ir") == 0 and smiles.count("Li") == 0 and smiles.count("Be") == 0:
        return False
